...
| `js/calculator-engine.js` | **[Task 131]** Pure calculation engine. Exposes `CALCULATOR_ENGINE.calcSinglePackage(item)`, `calcSeparate(items, dest)`, `calcConsolidated(items, dest)`. No formula changes — structural refactor and extension of original inline logic. **Sole source of calculator math** (legacy `js/calculator.js` was removed as dead code on 2026-05-02; it was never loaded by any page and held a stale duplicated tax table with the typo `repuestos_vehículo`). |
...
| `/admin/solicitudes/:id` | GET | **[Task 185]** Admin detail page. Shows customer block, product block, AI extraction snapshot with per-field confidence (amber highlight if < 0.80 or `needs_confirmation`), Estimado del sistema block, status history timeline, inline status update form, and **[Task 188]** Response Composer (embedded form for enviada/en_revision solicitudes to confirm price, availability, conditions, and send a structured response email; renders as read-only "Respuesta enviada" block once response_json is committed). **Calculator redesign**: the interactive shipping calculator section (air freight only) now shows a dark comparison hero (Envío por separado vs Consolidado CRBOX) with savings amount+%, pill-style dossier tab switcher (Consolidado / Por separado), per-product accordion cards with full line-item breakdowns, and an "Incluir en la respuesta" apply button. `lastBreakdown` now stores `grand_total_usd` = consolidated total, plus `separate_total_usd`, `savings_usd`, `savings_pct`. Calculation is triggered by button click (not on-input); state autosaves to localStorage keyed by SCB-ID (A-2). Response Composer includes a live email preview pane (`#cmp-preview-pane`) that renders a miniature HTML email in real time as the admin types — updated via debounced `_buildPrev`/`_schedPrev` on every field change (A-3). |
...
| `/api/package-group-confirm` | POST | **[Task 316/328]** Sends confirmation email to facturas@crbox.cr. Now also generates a unique `ack_token`, stores it on the group record, and embeds a "Confirmar recepción" magic link in the email body (HTML button + plain-text URL). |
| `/api/package-group-ack` | GET | **[Task 328]** No auth required. CRBOX staff click link from email. Looks up group by `ack_token`, checks 30-day expiry and `confirmation_sent` status, updates status to `received_by_crbox` with `receivedAt` timestamp, returns branded HTML page. Idempotent (clicking twice shows "ya confirmado" page). |
