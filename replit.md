...
- **Invoice `FileLocation` must be a path, NOT a full URL:** The CRBOX admin tool prepends `https://crbox.cr` to `FileLocation` when constructing both the open/download link and the image preview src. If `FileLocation` is a full URL (e.g. `https://crbox.cr/wp-content/uploads/...`), the result is `https://crbox.crhttps://crbox.cr/...` → host `crbox.crhttps` → "Servidor no encontrado". Always extract just the pathname: `new URL(fileUrl).pathname` → `/wp-content/uploads/...`. This is handled in the `saveBill` wrapper in `mis-paquetes.html` via the `_filePath` variable. Do not revert to `fileUrl` directly.
