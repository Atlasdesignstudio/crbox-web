...
- Personal: 3-step stepper (`.signup-stepper` + `.signup-step-panel` panels)
  ...
  2. **Identidad** — `id_type` (pill-toggle buttons: Cédula Nacional / DIMEX / Pasaporte; value written to a sr-only `<select name="id_type">` and validated in a custom `n===2` block since the select has no `required` attr to avoid native reportValidity on the hidden element), `id_number`, `birth_date` (→ `Consignee.BirthDate`, required by backend; displayed side-by-side with id_number), single `phone_number[]` (hidden `phone_type[]=movil`)
