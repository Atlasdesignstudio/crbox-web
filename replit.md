...
| `/api/ai/extract` | POST | AI product data extraction. Two-stage pipeline: (1) fetch page HTML via `_fetch_page` + Gemini HTML analysis; if page is bot-blocked (CAPTCHA/WAF), (2) Google Search fallback via `_call_gemini_search_fallback` using `types.Tool(google_search=...)` — Gemini searches Google to find name/price/category even when retailers (Amazon, Walmart, etc.) block direct scraping. Result model field is `gemini-2.5-flash-lite+search` when fallback is used. `extraction_warnings` includes a note when search fallback was used. Bot detection via `_is_bot_blocked()` checks 15+ CAPTCHA/WAF signals. |
