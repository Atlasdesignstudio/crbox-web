...
- **Invoice `FileLocation` must be the full absolute URL:** The CRBOX admin tool uses `FileLocation` directly for both the inline image preview and the open/print link — pass the full `https://crbox.cr/wp-content/uploads/...` URL returned by `saveBill`. Do not strip it to a path or add `.pathname` extraction. The server's `_handle_proxy_savebill` always returns a fully-qualified URL; `fileUrl` in `mis-paquetes.html` is correct as-is.
