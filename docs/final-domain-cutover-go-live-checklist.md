...
| 4 | New Replit custom domain target documented and configured | **Blocks DNS cutover** | ✅ **Confirmed 2026-05-16** | `crbox.cr` and `www.crbox.cr` added as custom domains in Replit. DNS target: A `34.111.179.208`. TXT verification records added to Route 53 2026-05-16: `crbox.cr` TXT appended (SPF preserved); `www.crbox.cr` TXT new record set created. Replit domains currently verifying. A records unchanged — `crbox.cr` and `www.crbox.cr` still point to `98.90.3.205`. At cutover: only the two A records change. Source: `docs/dns-cutover-operational-status.md` Section 6. |
| 5 | SSL certificate provisioned or auto-provisioning confirmed for `crbox.cr` and `www.crbox.cr` on Replit | **Blocks DNS cutover** | ⚠️ Verifying — TXT added; Replit verification in progress; SSL pre-provisioning expected before A record flip | TXT records in Route 53. Replit is verifying domain ownership. SSL certificate expected to pre-provision before the A record change, eliminating any SSL gap at cutover. Final confirmation is part of minute-0 smoke test after A records flip. Source: `docs/dns-cutover-operational-status.md` Section 7. |
...
| 7 | Old hosting preserved and confirmed reachable for rollback window | **Blocks DNS cutover (rollback safety)** | ✅ **Confirmed 2026-05-16** | Mathias confirmed ownership and preservation of EC2 instance `CrBox.cr V2` (`98.90.3.205`, us-east-1c, t3.medium, running, 3/3 status checks). Committed to keeping the instance live and reachable for 2–4 weeks post-cutover. Will not stop, terminate, decommission, or change the IP or security groups. No EC2 changes made. Source: `docs/dns-cutover-operational-status.md` Section 5. |
...
⚠️ Verifying: item 5 (SSL — TXT records in Route 53; Replit verification in progress; SSL pre-provisioning expected before A record flip)  
