...
|---|---|---|---|---|
| 1 | DNS owner identified and available for cutover window | **Blocks DNS cutover** | ⚠️ Open — Needs owner | Not documented anywhere in the codebase. This is the single hardest operational blocker. |
| 2 | Current DNS records for `crbox.cr`, `www.crbox.cr`, `clients.crbox.cr`, `admin.crbox.cr` exported and backed up | **Blocks DNS cutover** | ⚠️ Open | Required before any change; prevents irrecoverable loss if something goes wrong. |
| 3 | Old hosting target documented (URL / IP / provider) | **Blocks DNS cutover** | ⚠️ Open | Needed for rollback — can't revert to old hosting if the target is unknown. |
| 4 | New Replit custom domain target documented and configured | **Blocks DNS cutover** | ⚠️ Open | Replit must have the custom domain added before DNS is pointed to it. |
| 5 | SSL certificate provisioned or auto-provisioning confirmed for `crbox.cr` and `www.crbox.cr` on Replit | **Blocks DNS cutover** | ⚠️ Open | Replit provisions automatically for custom domains, but this must be confirmed before the cutover window opens. HTTPS must be valid immediately after propagation. |
| 6 | DNS TTL for `crbox.cr` and `www.crbox.cr` lowered to ~300 s at least 24 h before cutover | **Blocks DNS cutover (rollback risk)** | ⚠️ Open | If TTL is not lowered, a rollback may take hours to propagate. Risk can be explicitly accepted but must be documented. |
| 7 | Old hosting preserved and confirmed reachable | **Blocks DNS cutover (rollback safety)** | ⚠️ Open | Old hosting must not be decommissioned during the confidence period. |
| 8 | `clients.crbox.cr` confirmed reachable and returning valid API responses | **Blocks DNS cutover** | ❓ Not recently verified | All auth and portal data flows through this endpoint. Must be verified immediately before the cutover window. |
| 9 | `admin.crbox.cr` confirmed reachable | **Blocks DNS cutover** | ❓ Not recently verified | Internal admin tool. Must be verified before cutover to confirm it will not be affected. |
...
| 11 | Replit production deployment verified live and stable (not just preview) | **Blocks DNS cutover** | ❓ Not verified | The DNS record must point to the production deployment, not the dev preview URL. |
...
| 13 | Rollback owner assigned and confirmed available during cutover window | **Blocks DNS cutover (rollback safety)** | ⚠️ Open — Needs owner | Someone must be designated to execute the rollback procedure if a trigger fires. |
| 14 | Rollback communication channel confirmed | **Blocks DNS cutover (rollback safety)** | ⚠️ Open | A specific channel (e.g. WhatsApp group, Slack, phone) where the cutover team monitors during the window. |
