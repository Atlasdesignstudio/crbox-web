...
| 2 | Current DNS records exported and backed up | **Blocks DNS cutover** | ⚠️ Partial — A records confirmed; MX/TXT/NS require registrar login | A records confirmed 2026-05-14: `crbox.cr`/`www.crbox.cr` → `98.90.3.205`; `clients.crbox.cr`/`admin.crbox.cr` → `100.50.198.105`. Full zone backup must be exported from registrar before cutover. Source: `docs/dns-cutover-operational-status.md`. |
...
| 4 | New Replit custom domain target documented and configured | **Blocks DNS cutover** | ⚠️ Open — needs user confirmation | Replit deployment is configured (autoscale). Custom domain A record / CNAME target must be confirmed in Replit Settings → Custom Domains before DNS change. |
| 5 | SSL certificate provisioned or auto-provisioning confirmed for `crbox.cr` and `www.crbox.cr` on Replit | **Blocks DNS cutover** | ⚠️ Open — depends on item 4 | Replit auto-provisions SSL when custom domain is added and DNS points to Replit. Confirm custom domain is added in Replit before cutover. |
| 6 | DNS TTL for `crbox.cr` and `www.crbox.cr` lowered to ~300 s at least 24 h before cutover | **Blocks DNS cutover (rollback risk)** | ⚠️ Open — current TTL unknown | Current TTL could not be read from this environment. Must be checked and lowered in the DNS provider panel. If not lowered, rollback may take up to the full current TTL to propagate (risk must be explicitly accepted if skipped). |
| 7 | Old hosting preserved and confirmed reachable for rollback window | **Blocks DNS cutover (rollback safety)** | ⚠️ Partially confirmed — site is live; longevity needs user confirmation | Old site at `98.90.3.205` is serving. Need confirmation the AWS instance will remain active for 2–4 weeks post-cutover and who controls it. |
