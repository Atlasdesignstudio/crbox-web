...
|---|---|---|---|---|
| 1 | DNS owner / Route 53 access holder identified and available for cutover window | **Blocks DNS cutover** | ⚠️ Partial — registrar confirmed; Route 53 holder not yet identified | Registrar: dominios.cr / NIC Costa Rica. Authoritative DNS: **AWS Route 53** (nameservers: `ns-1400.awsdns-47.org`, `ns-148.awsdns-18.com`, `ns-1718.awsdns-22.co.uk`, `ns-725.awsdns-26.net`). All record changes must be made in Route 53, NOT in dominios.cr. The person with Route 53 hosted zone access must be identified and confirmed available. Source: user confirmation + `docs/dns-cutover-operational-status.md`. |
...
| 6 | DNS TTL for `crbox.cr` and `www.crbox.cr` lowered to ~300 s at least 24 h before cutover | **Blocks DNS cutover (rollback risk)** | ❌ Open — current TTL unknown; must be checked in Route 53 | Current TTL cannot be determined from this environment. The Route 53 access holder must check the current TTL and lower it to 300 s at least 24 h before the cutover window. If not lowered: rollback propagation takes up to the full current TTL (e.g. 1 h for 3600 s, 24 h for 86400 s). Any decision to skip must be explicitly accepted and documented. Source: user confirmation 2026-05-14 + `docs/dns-cutover-operational-status.md`. |
...
| 11 | Replit production deployment verified live and stable (not just preview) | **Blocks DNS cutover** | ❌ Open — only dev/preview exists so far | User confirmed 2026-05-14: only the dev/preview deployment is active. A production deployment must be published and verified stable before DNS is pointed to it. |
...
✅ Confirmed resolved: items 3, 8, 9, 16  
⚠️ Partially confirmed: items 1, 2, 14  
❌ Open: items 4, 5, 6, 7, 11, 12, 13  
