...
const {
  ...
  summaryLines: ga4MonitoringReadinessSummary
} = require('./ga4-monitoring-readiness');
const {
  runGa4EventProcessingValidation,
  summaryLines: ga4EventProcessingValidationSummary
} = require('./ga4-event-processing-validation');
const {
  runGa4MonitoringDashboard,
  summaryLines: ga4MonitoringDashboardSummary
} = require('./ga4-monitoring-dashboard');
const {
...
const {
  ...
  summaryLines: googleAdsImportPayloadReviewSummary
} = require('./google-ads-import-payload-review');
const {
...
const {
  ...
  summaryLines: googleAdsControlledImportApplyExecutionSummary
} = require('./google-ads-controlled-import-apply-execution');
const {
  runGoogleAdsGa4ImportDiagnostic,
  summaryLines: googleAdsGa4ImportDiagnosticSummary
} = require('./google-ads-ga4-import-diagnostic');
const {
  runGoogleAdsPostApplyMonitoring,
  summaryLines: googleAdsPostApplyMonitoringSummary
} = require('./google-ads-post-apply-monitoring');
const {
  runGoogleAdsConversionMonitoringWindow,
  summaryLines: googleAdsConversionMonitoringWindowSummary
} = require('./google-ads-conversion-monitoring-window');
...
async function main() {
  ...
  switch (command) {
    ...
    case 'ga4:monitoring-readiness': {
      const monitoringReadiness = await runGa4MonitoringReadiness(root);
      for (const line of ga4MonitoringReadinessSummary(monitoringReadiness)) {
        console.log(maskSecretsInText(line));
      }
      return;
    }
    case 'ga4:event-processing-validation': {
      const eventProcessingValidation = await runGa4EventProcessingValidation(root);
      for (const line of ga4EventProcessingValidationSummary(eventProcessingValidation)) {
        console.log(maskSecretsInText(line));
      }
      return;
    }
    case 'ga4:monitoring-dashboard': {
      const monitoringDashboard = runGa4MonitoringDashboard(root);
      for (const line of ga4MonitoringDashboardSummary(monitoringDashboard)) {
        console.log(maskSecretsInText(line));
      }
      return;
    }
    case 'paid-media:conversion-map': {
      const conversionMap = runPaidMediaConversionMap(root);
      for (const line of paidMediaConversionMapSummary(conversionMap)) {
        console.log(maskSecretsInText(line));
      }
      return;
    }
    ...
    case 'google-ads:payload-review': {
      const payloadReview = runGoogleAdsImportPayloadReview(root);
      for (const line of googleAdsImportPayloadReviewSummary(payloadReview)) {
        console.log(maskSecretsInText(line));
      }
      return;
    }
    case 'google-ads:account-preflight': {
      const accountPreflight = await runGoogleAdsAccountPreflight(root);
      for (const line of googleAdsAccountPreflightSummary(accountPreflight)) {
        console.log(maskSecretsInText(line));
      }
      return;
    }
    ...
    case 'google-ads:controlled-import-apply': {
      const execution = await runGoogleAdsControlledImportApplyExecution(root, process.argv.slice(3));
      for (const line of googleAdsControlledImportApplyExecutionSummary(execution)) {
        console.log(maskSecretsInText(line));
      }
      if (execution.report.exitCode) process.exitCode = execution.report.exitCode;
      return;
    }
    case 'google-ads:ga4-import-diagnostic': {
      const diagnostic = await runGoogleAdsGa4ImportDiagnostic(root);
      for (const line of googleAdsGa4ImportDiagnosticSummary(diagnostic)) {
        console.log(maskSecretsInText(line));
      }
      return;
    }
    case 'google-ads:post-apply-monitoring': {
      const monitoring = await runGoogleAdsPostApplyMonitoring(root);
      for (const line of googleAdsPostApplyMonitoringSummary(monitoring)) {
        console.log(maskSecretsInText(line));
      }
      return;
    }
    case 'google-ads:conversion-monitoring-window': {
      const monitoringWindow = await runGoogleAdsConversionMonitoringWindow(root);
      for (const line of googleAdsConversionMonitoringWindowSummary(monitoringWindow)) {
        console.log(maskSecretsInText(line));
      }
      return;
    }
