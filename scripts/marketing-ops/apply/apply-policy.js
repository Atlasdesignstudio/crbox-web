...
const GTM_EXECUTION_READY_MESSAGE = 'GTM controlled create execution is enabled for explicitly selected approved workspace actions.';
const ALLOWED_GA4_CREATE_ACTIONS = Object.freeze(new Set(['create_custom_dimension', 'mark_key_event']));
const ALLOWED_GTM_CREATE_ACTIONS = Object.freeze(new Set(['create_data_layer_variable', 'create_custom_event_trigger']));
...
function rejectionReasonsForAction(action, targetPlatform) {
  const reasons = [];
  ...
  } else if (targetPlatform === 'gtm') {
    if (action.platform !== 'gtm') reasons.push('Only GTM actions may be executed in Phase 2F.');
    if (!ALLOWED_GTM_CREATE_ACTIONS.has(action.action)) reasons.push('Action is not in the GTM controlled-create allowlist.');
  } else {
    reasons.push('Unsupported controlled-create target platform.');
  }
...
function evaluateApplyPolicy({ validation, scope = 'all', argv = [], env = process.env, requestedMode = 'preview' }) {
  ...
  const gtmCreateFlagEnabled = env.MARKETING_AGENT_GTM_CREATE_ENABLED === 'true';
  ...
  const isGtmCreateMode = requestedMode === 'gtm_controlled_create';
  const targetPlatform = isGa4CreateMode ? 'ga4' : (isGtmCreateMode ? 'gtm' : '');
  const rejectedActions = selectedActions
    .map((action) => ({ action, reasons: rejectionReasonsForAction(action, targetPlatform) }))
    .filter((item) => item.reasons.length > 0);
  const controlledCreateRequested = mode === 'controlled_create';
  const platformProvided = isGtmCreateMode ? options.platform === 'gtm' : options.platform === 'ga4';
  ...
  if (isGtmCreateMode) {
    ...
    if (!controlledCreateRequested) gateFailures.push('MARKETING_AGENT_MODE must be controlled_create.');
    if (!writesFlagEnabled) gateFailures.push('MARKETING_AGENT_ENABLE_WRITES must be true.');
    if (!gtmCreateFlagEnabled) gateFailures.push('MARKETING_AGENT_GTM_CREATE_ENABLED must be true.');
    if (!platformProvided) gateFailures.push('CLI flag --platform gtm is required.');
    if (!options.confirmHumanApproval) gateFailures.push('CLI flag --confirm-human-approval is required.');
    if (!actionSelectionProvided) gateFailures.push('CLI flag --all or at least one --action-id is required.');
    if (selectedActions.length === 0) gateFailures.push('At least one selected action must match the dry-run plan.');
    if (eligibleGa4Actions.length > 0) gateFailures.push('GA4 actions cannot be selected for GTM controlled create.');
    if (selectedActions.some((action) => action.platform !== 'gtm')) gateFailures.push('Only GTM actions may be selected.');
    if (rejectedActions.length > 0) gateFailures.push('One or more selected actions are rejected by policy.');
  }

  const executionEnabled = (isGa4CreateMode || isGtmCreateMode) && gateFailures.length === 0;

  return {
    mode,
    executionMode: isGa4CreateMode ? 'ga4_controlled_create' : (isGtmCreateMode ? 'gtm_controlled_create' : (requestedMode === 'validation' ? 'validation' : 'preview')),
    writesFlagEnabled,
    gtmCreateFlagEnabled,
    ...
    executionReadyMessage: executionEnabled ? (isGtmCreateMode ? GTM_EXECUTION_READY_MESSAGE : GA4_EXECUTION_READY_MESSAGE) : '',
    ...
    eligibleGa4Actions,
    eligibleGtmActions,
    rejectedActions: (isGa4CreateMode || isGtmCreateMode) ? rejectedActions : [],
    gateFailures,
